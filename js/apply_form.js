...
jQuery(document).ready(function () {
    ...
    jQuery(".af_clss").each(function () {
        var dialog_elm = "dialog_" + form_count;
        /*dialog id*/
        var applybtn_elm = "afbtn_" + form_count;
        /*apply form button */
        var form_elm = 'afform_' + form_count;
        /*form id */
    ...
    jQuery(".apply_form").click(function () {
        ...
        jQuery('.dialogcls').text('Please wait...');
            ...
            success: function (response) {
                if (response['invalid']) {
                    jQuery.each(response.invalid, function (i, n) {
                        jQuery(form_id).find("input[name='" + i + "']").parent().append('<span class="wp_af_validation_error" style="padding-left:10px">' + n + '</span>');
                    });
                        ...
                        jQuery.each(response.file_response, function (i, n) {
                            ...
                            if (i.substring(0, 16) == "file_upload_path") {
                                ...
                                if (invalid_file_value == 'Invalid file' || invalid_file_value == '') {
                                    jQuery("span#file_select_err_" + container_arr[1]).html('');   // 02-june-15
                                    jQuery(form_id).find("input[name='" + i + "']").parent().parent().append('<span class="wp_af_validation_error" style="padding-left:10px">' + n + '</span>');
                                    //return false;    //CRINCH V2
                                }
                    ...
                    setTimeout(function () {
                        ...
                        jQuery('.dialogcls').text(response['response']);
                    }, 7000);
...
function afterFileCallMail() {
    var this_elm = jQuery(".apply_form");
    var apply_btn_id = jQuery(this_elm).attr('id');
    var btn_arr = new Array();
    btn_arr = apply_btn_id.split("_");
    var form_dialog = "#dialog_" + btn_arr[1];
        ...
        jQuery.ajax({
            ...
            success: function (response) {
                ...
                if (response['debug']) {
                    ...
                    /*handles response messages or when debug mode is off*/
                    var x = 0;
